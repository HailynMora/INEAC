...
class HabilitacionesController extends Controller
{
    ...
    public function estudiantil(){
        $idl=auth()->id();
        $idc = DB::table('estudiante')->where('estudiante.id_usuario', '=', $idl)
                    ...
                    ->first();
        $asig = DB::table('notas')->where('notas.id_estudiante',$idc->ides)
                    ...
                    ->get();
        return view('estudiantes.certificadoes')->with('idc', $idc)->with('asig', $asig);
    }
}
