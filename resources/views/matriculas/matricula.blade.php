...
<div class="alert text-center" role="alert" style="background-color: #283593; color:#ffffff;">
 <h3>Buscar Estudiante A Matricular</h3>
</div>
...
<div class="container-fluid">
<a id="listado" type="button" class="btn btn-info">Visualizar</a>
</div>
        ...
        $(function ()
        {
            ...
            $(document).on('keyup', '#search-form .search', function ()
            {
                if($(this).val().length > 0)
                {
                    ...
                    $.get("{{ route('postmatricular') }}", {search: search}, function (res)
                    {
                        $('#results').html(res);
                    });
            ...
            $(document).on('click', '.post-link', function ()
            {
                ...
                $.get("{{ route('showmatricular') }}", {id: postId}, function (res)
                {
                    $('#results').empty();
                    $('.search').val('');
                    $('#post').html(res);
